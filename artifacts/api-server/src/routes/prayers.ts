...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const { isCrisis, keywords } = detectCrisisKeywords(parsed.request);
    ...
    const result = await db.insert(prayers).values({
      ...
      flagged_keywords: isCrisis ? keywords.join(', ') : null,
    });

    res.status(201).json({
      success: true,
      crisis_flag: isCrisis,
      message: isCrisis ? 'Prayer submitted. Crisis detected. 988 resources available.' : 'Prayer submitted.',
    });
  } catch (error) {
